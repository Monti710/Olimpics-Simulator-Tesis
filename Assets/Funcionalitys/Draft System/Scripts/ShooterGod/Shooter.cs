using UnityEngine;
using UnityEngine.InputSystem; // Para usar InputActionProperty
...
public class Shooter : MonoBehaviour
{
    ...
    [Header("Referencias")]
    public SceneMan sceneMan; // Referencia al script SceneMan
    private int currentShots; // Número de balas restantes
    ...
    void Start()
    {
        // Inicializamos el número de balas en Shooter
        currentShots = sceneMan.maxShots; // Asignamos el valor de las balas desde SceneMan
    }
    ...
    void Update()
    {
        ...
        if (shouldFire && currentShots > 0)
        {
            Fire(); // Llamamos al método Fire cuando se debe disparar
            _cooldown = fireCooldown; // Reinicia el cooldown

            // Disminuimos las balas
            currentShots--;
            sceneMan.UpdateShotDisplay(currentShots); // Actualizamos la visualización de las balas
    ...
    {
        // Asigna la cantidad de balas iniciales
